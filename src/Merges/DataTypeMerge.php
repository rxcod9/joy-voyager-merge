...
use Joy\VoyagerMerge\Events\BreadDataMergeed;
...
class DataTypeMerge implements
    ToModel,
    WithHeadingRow,
    WithProgressBar,
    WithValidation,
    ...
    WithEvents
{
    ...
    public function registerEvents(): array
    {
        return [
            ...
            AfterMerge::class => function (AfterMerge $event) {
                event(new BreadDataMergeed($this->dataType, $this->input));
            },
        ];
    }
    ...
    public function uniqueBy()
    {
        ...
        $dataTypeUniqueColumn = config(
            'joy-voyager-merge.unique_column.' . $this->dataType->slug
        );
    ...
    {
        $isValidationEnabled = config('joy-voyager-merge.validation', false) === true;
    ...
    public function model(array $item)
    {
        ...
        foreach ($this->dataType->rows as $row) {
            if ($data->hasSetMutator($row->field . '_merge')) {
                $data->{$row->field . '_merge'} = ($item[$row->field] ?? null);
            }
